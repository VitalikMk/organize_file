...
fn organize_dir(dir_path: PathBuf) {
    if !dir_path.exists() {
        println!("Dir \"{}\" does't niema twoego fila", dir_path.display());
    } else {
        ...
        for file in dir_files{
            if let Ok(file) = file {
                ...
                let file_extension = match file.path().extension() {
                    None => {
                        println!("Can't get extention of the file: \"{}\" ", file.path().display());
                        continue;
                    }
...
fn main() {
    loop {
        ...
        println!("Time to organize: {}sekund", now.elapsed().as_secs_f64())
    }
}
